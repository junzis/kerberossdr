...
class SignalProcessor(QtCore.QThread):
    ...
    def __init__(self, parent=None, module_receiver=None):
        ...
        self.cfar_win_params = [
            10,
            10,
            4,
            4,
        ]  # [Est. win length, Est. win width, Guard win length, Guard win width]
        ...
        self.spectrum = np.ones(
            (self.channel_number + 1, self.spectrum_sample_size), dtype=np.float32
        )
        self.xcorr = np.ones(
            (self.channel_number - 1, self.xcorr_sample_size * 2), dtype=np.complex64
        )
        self.phasor_win = 2 ** 10  # Phasor plot window
        self.phasors = np.ones(
            (self.channel_number - 1, self.phasor_win), dtype=np.complex64
        )
    ...
    def run(self):
        ...
        while self.run_processing:
            ...
            if self.en_spectrum:
                self.spectrum[0, :] = (
                    np.fft.fftshift(
                        np.fft.fftfreq(self.spectrum_sample_size, 1 / self.fs)
                    )
                    / 10 ** 6
                )
            ...
            if self.en_calib_iq:
                ...
                for m in range(self.channel_number):
                    self.module_receiver.iq_corrections[m] *= np.size(
                        self.module_receiver.iq_samples[0, :]
                    ) / (
                        np.dot(
    ...
    def estimate_DOA(self):
        ...
        if self.DOA_ant_alignment == "UCA":
            ...
            if self.en_DOA_MUSIC:
                self.DOA_MUSIC_res = de.DOA_MUSIC(
                    R, scanning_vectors, signal_dimension=1
                )
    ...
    def PR_processing(self):
        ...
        self.RD_matrix = det.cc_detector_ons(
            ref_ch,
            surv_ch,
            self.fs,
            self.max_Doppler,
        ...
        if self.en_PR_autodet:
            self.hit_matrix = CA_CFAR(
                self.RD_matrix, self.cfar_win_params, self.cfar_threshold
            )
