...
int main()
{
    fprintf(stderr, "sim: Hydra test started \n");
    int success = 1;
    char filename[32];
    size_t read_len;
    ...
    for(int m=0; m< CH_NO; m++)
    {
        sprintf(filename, "data/%d.iq", m);
        if((fds[m] = fopen(filename, "rb")) == NULL) {
            fprintf(stderr, "sim: Could not open file \n");
            success=0;
        }
    }
    if(success != 1)
    ...
    fprintf(stderr,"sim: All files opened\n");


    for(int b=0; b<500; b++)
    {
        fprintf(stderr, "sim: Transfering block:%d ",b);
        usleep(1000000);
        for(int m=0; m< CH_NO; m++)
        {
            ...
            {
                fprintf(stderr, "sim:  [ FAILED ]\n");
                fprintf(stderr, "sim: ERROR: Read bytes: %zu\n",read_len);
                success = 0;
                break;
            }
    ...
    {
        if(fclose(fds[m])==EOF)
            fprintf(stderr,"sim:  File close failed closed\n");
    }
    fprintf(stderr,"sim: All files closed\n");
    ...
    fprintf(stderr, "sim: Hydra test finished\n");
    return 0;
}
