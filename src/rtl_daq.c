...
#define CFN "tmp/rec_control_fifo" /* Receiver control FIFO name */
...
void * fifo_read_tf(void* arg)
{
    ...
    */
    ...
    fprintf(stderr,"rtl_daq: FIFO read thread is running\n");
    if(fd!=0)
        fprintf(stderr,"rtl_daq: FIFO opened\n");
    else
        fprintf(stderr,"rtl_daq: FIFO open error\n");
    while(!exit_flag){
        ...
        if( (uint8_t) signal == 2)
        {
            //fprintf(stderr,"rtl_daq: Signal2: FIFO read thread exiting \n");
            exit_flag = 1;
        }
        else
        {
        ...
        if( (char) signal == 'r')
        {
            //fprintf(stderr,"rtl_daq: Signal 'r': Reconfiguring tuner \n");
            ...
            //fprintf(stderr,"rtl_daq: [ INFO ] Center freq: %u MHz\n", ((unsigned int) center_freq_read/1000000));
            //fprintf(stderr,"rtl_daq: [ INFO ] Sample rate: %u MSps\n", ((unsigned int) sample_rate_read/1000000));
            //fprintf(stderr,"rtl_daq: [ INFO ] Gain: %d dB\n",(gain_read/10));
        ...
        {
            //fprintf(stderr,"rtl_daq: Signal 'n': Turn on noise source \n");
        ...
        {
            //fprintf(stderr,"rtl_daq: Signal 'f': Turn off noise source \n");
