...
#define CFN "tmp/sync_control_fifo" // Name of the gate control fifo - Control FIFO name
 ...
 *
 */
    fprintf(stderr,"sync: FIFO read thread is running\n");
    (void)arg;
    FILE * fd = fopen(CFN, "r"); // FIFO descriptor
    if(fd!=0)
        fprintf(stderr,"sync: FIFO opened\n");
    else
        fprintf(stderr,"sync: FIFO open error\n");
    uint8_t signal;
    while(1){
        fread(&signal, sizeof(signal), 1, fd);
        if( (uint8_t) signal == 1)
        {
            fprintf(stderr,"sync: Signal1: trigger\n");
            trigger++;
            sem_wait(&trigger_sem);
        }
        else if( (uint8_t) signal == 2)
        {
            fprintf(stderr,"sync: Signal2: FIFO read thread exiting \n");
            exit_flag = 1;
            break;
        }
        else if( (char) signal == 'd')
        {
            //fprintf(stderr,"sync: Signal 'd': Updating delay values \n");
            fread(delays, sizeof(*delays), 4, fd);
            for(int m=0; m < CHANNEL_NO; m++)
                if(abs(delays[m]) < BUFFER_SIZE)
                {
                    //fprintf(stderr,"sync: [ INFO ] Channel %d, delay value %d\n",m,delays[m]);
                    delays[m] *=2; // I and Q !
                }
                else
                {
                    //fprintf(stderr,"sync: [ ERROR ] Delay value over buffer size: %d, Setting to zero\n",delays[m]);
                    delays[m] = 0;
                    //fprintf(stderr,"sync: [ WARNING ]Channel %d, delay value %d\n",m,delays[m]);
                }
...
int main(int argc, char** argv)
{
    ...
    for(int m=0;m<CHANNEL_NO;m++)
    {
        ...
        //fprintf(stderr,"sync: ch: %d, Buff pointer: %p\n",m,(sync_buffers + m * sizeof(*sync_buffers))->circ_buffer);
    }

    fprintf(stderr,"sync: Start delay sync test\n");
    while(!exit_flag)
    {
        ...
        for(int m=0; m<CHANNEL_NO; m++)
        {
            ...
            if(read_size == 0)
            {
                //fprintf(stderr,"sync: Read error at channel: %d, wr_index: %d\n",m, write_index);
                exit_flag = 1;
                break;
            }
            ...
            /* Log */
            //fprintf(stderr,"sync: Channel: %d, wr_index: %d\n",m,write_index);
            //fprintf(stderr,"sync: Delay %d\n", delay);
    ...
    fprintf(stderr,"sync: [ EXIT ] Sync block exited\n");
    return 0;
