...
receiverPath = os.path.join(rootPath, "receiver")
...
class MainWindow(QMainWindow, Ui_MainWindow):
    def __init__(self, parent=None):
        ...
        self.spectrum_ch1_curve = self.plotWidget_spectrum_ch1.plot(
            x, y[0], clear=True, pen=(255, 199, 15)
        )
        self.spectrum_ch2_curve = self.plotWidget_spectrum_ch2.plot(
            x, y[1], clear=True, pen="r"
        )
        self.spectrum_ch3_curve = self.plotWidget_spectrum_ch3.plot(
            x, y[2], clear=True, pen="g"
        )
        self.spectrum_ch4_curve = self.plotWidget_spectrum_ch4.plot(
            x, y[3], clear=True, pen=(9, 237, 237)
        )
        ...
        self.DOA_res_fd = open(
            "/ram/DOA_value.html", "w"
        )  # DOA estimation result file descriptor
        ...
        self.plotWidget_DOA.plot(
            x, y[0], pen=pg.mkPen((255, 199, 15), width=2), name="Bartlett"
        )
        ...
        self.plotWidget_DOA.plot(
            x, y[3], pen=pg.mkPen((9, 237, 237), width=2), name="MUSIC"
        )
        ...
        self.pushButton_dumping_data.clicked.connect(self.pb_dumping_data_clicked)
    ...
    def pb_rec_reconfig_clicked(self):
        ...
        self.module_signal_processor.center_freq = (
            self.doubleSpinBox_center_freq.value() * 10 ** 6
        )
    ...
    def set_DOA_params(self):
        ...
        self.module_signal_processor.DOA_ant_alignment = (
            self.comboBox_antenna_alignment.currentText()
        )
        ...
        self.module_signal_processor.DOA_sample_size = (
            2 ** self.spinBox_DOA_sample_size.value()
        )
    ...
    def set_PR_params(self):
        ...
        self.module_signal_processor.cfar_threshold = (
            self.doubleSpinBox_cfar_threshold.value()
        )

        # Set Time-domain clutter fitler parameters
        self.module_signal_processor.td_filter_dimension = (
            self.spinBox_td_filter_dimension.value()
        )
    ...
    def pb_proc_control_clicked(self):
        if self.pushButton_proc_control.text() == "Start processing":
            ...
            if self.module_receiver.dump_flag:
                self.module_receiver.dump_flag = False
                self.pushButton_dumping_data.setText("Start dumping data")

            self.module_signal_processor.stop()

    def pb_dumping_data_clicked(self):
        if self.module_receiver.dump_flag:
            self.module_receiver.dump_flag = False
            self.pushButton_dumping_data.setText("Start dumping data")

        elif not self.module_receiver.dump_flag:
            self.module_receiver.dump_flag = True
            self.pushButton_dumping_data.setText("Stop dumping data")
    ...
    def power_level_update(self, over_drive_flag):
        if over_drive_flag:
            ...
            green_text = (
                '<span style=" font-size:8pt; font-weight:600; color:#01df01;" >'
            )
            green_text += "OK"
            green_text += "</span>"
    ...
    def delay_plot(self):
        ...
        self.plotWidget_sync_absx.plot(
            delay_label, xcorr12[N - M : N + M + 1], pen=(255, 199, 15)
        )
        ...
        self.plotWidget_sync_sampd.plot(
            self.module_signal_processor.delay_log[0, :], pen=(255, 199, 15)
        )
        self.plotWidget_sync_sampd.plot(
        ...
        self.plotWidget_sync_phasediff.plot(
            self.module_signal_processor.phase_log[0, :], pen=(255, 199, 15)
        )
        self.plotWidget_sync_phasediff.plot(
    ...
    def DOA_plot_helper(
        ...
        DOA_data = np.divide(
            np.abs(DOA_data), np.max(np.abs(DOA_data))
        )  # normalization
        ...
        plot = self.plotWidget_DOA.plot(
            incident_angles, DOA_data, pen=pg.mkPen(color, width=2)
        )
        return DOA_data
    ...
    def DOA_plot(self):
        ...
        if self.module_signal_processor.en_DOA_Bartlett:
            ...
            plt = self.DOA_plot_helper(
                Bartlett, thetas, log_scale_min=-50, color=(255, 199, 15)
            )
        ...
        if len(DOA_results) != 0:
            ...
            # Combined Graph (beta)
            COMBINED_LOG = self.DOA_plot_helper(
                COMBINED, thetas, log_scale_min=-50, color=(163, 64, 245)
            )
    ...
    def RD_plot(self):
        ...
        if not self.checkBox_en_autodet.checkState():
            ...
            CAFMatrix = CAFMatrix / np.amax(
                CAFMatrix
            )  # Noramlize with the maximum value

            # Peak hold for PR
            if self.en_peakhold:
                CAFMatrixNew = np.maximum(
                    self.CAFMatrixOld, CAFMatrix
                )  # 1 * self.CAFMatrixOld + 1 * CAFMatrix
            ...
            plotPRImage = scipy.ndimage.zoom(
                CAFMatrixLog, self.PR_interp_factor, order=3
            )
            self.img_PR.clear()
